...
- **其余 5 家公司按相同口径扩展**——优先复用 `extract_longi_financials.py` 和 `extract_gree_financials.py` 的"按 y 重排 + startswith/in 双模式"框架；新公司首份年报需重新定位合并三大表起始页、单位口径，并核对行名变体（"所有者权益合计" vs "股东权益合计" 等）。
